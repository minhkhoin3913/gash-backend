...
router.post('/', authenticateJWT, authorizeRole(['admin', 'manager']), async (req, res) => {
  try {
    ...
    if (!mongoose.isValidObjectId(pro_id) || !mongoose.isValidObjectId(color_id) || 
        !mongoose.isValidObjectId(size_id) || !mongoose.isValidObjectId(image_id)) {
      return res.status(400).json({ message: 'Invalid product, color, size, or image ID' });
    }
...
router.get('/', async (req, res) => {
  try {
    ...
    const query = pro_id ? { pro_id: mongoose.Types.ObjectId(pro_id) } : {};
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const variant = await ProductVariants.findById(req.params.id)
      .populate('pro_id', 'Pro_name')
      .populate('color_id', 'color_name')
      .populate('size_id', 'size_name')
      .populate('image_id', 'imageURL');
...
router.put('/:id', authenticateJWT, authorizeRole(['admin', 'manager']), async (req, res) => {
  try {
    ...
    const variant = await ProductVariants.findByIdAndUpdate(
      req.params.id,
      { ...(pro_id && { pro_id }), ...(color_id && { color_id }), 
